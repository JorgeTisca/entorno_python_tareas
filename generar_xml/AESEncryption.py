...
class AES128Encryption:
    ...
    pkcs5_padding = lambda x, y: (x + (y - len(x) % y) * chr(y - len(x) % y)).encode(
        "utf-8"
    )
    ...
    def encrypt(self, plaintext: str, hex_key: str) -> str:
        ...
        ciphertext = (
            encryptor.update(AES128Encryption.pkcs5_padding(plaintext, len(iv)))
            + encryptor.finalize()
        )
    ...
    def decrypt(self, hex_key: str, b64_iv_ciphertext: str) -> str:
        ...
        print("Len: " + str(l))
        raw_iv = byte_iv_ciphertext[
            0:16
        ]  # The first 32 chars of hex string that correspond to 16 bytes
